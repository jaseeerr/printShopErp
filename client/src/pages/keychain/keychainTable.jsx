...
// Main component to display all sections' data
const KeychainTable = ({ data }) => {
  return (
    <div className="container mx-auto p-8">
      <h1 className="text-3xl font-bold text-center mb-8">Data Overview</h1>
      <div className="w-full flex flex-wrap">
        <PricingTable title="Material Pricing" items={transformData(data.material || {})} />
        <PricingTable title="Printing Pricing" items={transformData(data.printing || {})} />
        <PricingTable title="Print Type Pricing" items={transformData(data.printType || {})} />
        <PricingTable title="Shape Pricing" items={transformData(data.shape || {})} />
      </div>
    </div>
  );
};
